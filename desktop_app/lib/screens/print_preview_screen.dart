...
class _PrintPreviewScreenState extends State<PrintPreviewScreen> {
  ...
  Future<void> _fetchAndDecrypt() async {
    ...
    try {
      ...
      // 2. Get Local Private Key using current user's email
  ...
  Future<void> _securePrintTo(Printer printer) async {
    ...
    try {
      ...
      // Update status to PRINT_COMPLETED after successful print
      await apiService.updateFileStatus(
        widget.fileId,
        'PRINT_COMPLETED',
        authService.accessToken!,
      );
      print('Status updated to PRINT_COMPLETED');
      
      // Success - Delete file
      if (mounted) {
        setState(() {
          _decryptedBytes = null; // Clear from RAM immediately
        });
      }
