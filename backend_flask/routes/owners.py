...
@owners_bp.route('/delete-account', methods=['DELETE'])
@token_required
def delete_account():
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        cursor.execute("DELETE FROM sessions WHERE user_id = ?", (owner_id,))
        
        try:
             cursor.execute("DELETE FROM feedback WHERE user_id = ?", (owner_id,))
        except Exception:
             pass # Table might not exist yet if no feedback sent
        ...
        print(f"DEBUG: Deleted account for owner {owner_id}")
        return jsonify({'success': True, 'message': 'Account deleted successfully'})
        
    except Exception as e:
        conn.rollback()
        print(f"Delete Account Error: {e}")
        return jsonify({'error': True, 'message': 'Deletion failed'}), 500
    finally:
        cursor.close()
        release_db_connection(conn)
