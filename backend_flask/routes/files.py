...
# @token_required  <-- Disabled for testing
def list_files():
    # Hardcoded for testing - in production use g.user from token
    user_id = 'test-user-id-for-development'
    role = 'user'  # Default to user role for mobile app
    ...
    try:
        if role == 'user':
            ...
            query = """SELECT id, file_name, file_size_bytes, created_at, is_printed, printed_at, status, status_updated_at, rejection_reason 
                       FROM files WHERE (is_deleted = 0 OR status = 'REJECTED') AND user_id = ? ORDER BY created_at DESC LIMIT 100"""
            cursor.execute(query, (user_id,))
        elif role == 'owner':
            # Include REJECTED files even if deleted, so desktop app can show rejection status
            query = """SELECT id, file_name, file_size_bytes, created_at, is_printed, printed_at, status, status_updated_at, rejection_reason 
                       FROM files WHERE (is_deleted = 0 OR status = 'REJECTED') AND owner_id = ? ORDER BY created_at DESC LIMIT 100"""
            cursor.execute(query, (user_id,))
        else:
...
def delete_file(file_id):
    ...
    try:
        ...
        cursor.execute("SELECT owner_id, is_deleted, status, file_name FROM files WHERE id = ?", (file_id,))
        row = cursor.fetchone()
        if not row:
             return jsonify({'error': 'File not found'}), 404
        ...
        # Preserve APPROVED or REJECTED status, otherwise set to CANCELLED
        if current_status in ['APPROVED', 'REJECTED']:
            # Keep the current status (APPROVED/REJECTED)
            cursor.execute("""
                UPDATE files 
        ...
        # Notify via SSE
        try:
            sse_manager.publish(str(row[0]), "status_update", {
                "file_id": file_id,
                "file_name": file_name,
                "status": final_status,
